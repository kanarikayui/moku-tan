...
function record(stats, direction, isCorrect, dateKey = '2026-08-10') {
  return recordAnswer(stats, { direction, isCorrect, dateKey });
}
...
test('総計と方向別集計が正しい', () => {
  ...
  assert.deepEqual(stats.total, { asked: 4, correct: 3 });
  assert.deepEqual(stats.byDirection.en2ja, { asked: 2, correct: 1 });
  assert.deepEqual(stats.byDirection.ja2en, { asked: 2, correct: 2 });
});
...
test('日別集計が日付ごとに分かれる', () => {
  ...
  assert.deepEqual(stats.daily['2026-08-09'], { asked: 1, correct: 1 });
  assert.deepEqual(stats.daily['2026-08-10'], { asked: 2, correct: 1 });
});
...
test('日別グラフ用の系列は指定日を欠かさず返す', () => {
  ...
  assert.deepEqual(series[0], { dateKey: '2026-08-09', asked: 0, correct: 0, accuracy: null });
  assert.deepEqual(series[1], { dateKey: '2026-08-10', asked: 1, correct: 1, accuracy: 1 });
});
